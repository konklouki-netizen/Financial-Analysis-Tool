# app.py (v1.27 - Τελική Διόρθωση PDF Bytes)
...
# === Ρύθμιση Σελίδας ===
st.set_page_config(
    page_title="Financial Analysis Tool v1.27", # <--- Νέα έκδοση
    page_icon="📊",
    layout="wide" 
)
...
st.sidebar.title("📊 Εργαλεία Ανάλυσης (v1.27)") # <--- Νέα έκδοση
...
st.title("📊 Financial Analysis Dashboard (v1.27)") # <--- Νέα έκδοση
...
if st.session_state.analysis_inputs:
    ...
    with st.spinner("Δημιουργία αναφοράς PDF..."):
        ...
        # === v1.26 FIX: Αφαίρεση της λάθος μετατροπής ===
        # Η συνάρτηση create_pdf_report (v2.2) θα επιστρέφει ήδη bytes
        pdf_bytes_fixed = pdf_data_raw 
        # === === === === ===
