...
def get_industry_tickers(industry_name: str, sector_name: str) -> List[str]:
    """
    v3.8: Χρησιμοποιεί το Finviz Screener για να βρει όλα τα tickers ενός Κλάδου (Industry).
    Επιστρέφει μια λίστα με tickers.
    """
    if not industry_name or not sector_name:
        print("⚠️ Info: Δεν δόθηκαν Industry ή Sector Name για αναζήτηση κλάδου.")
        return []
    try:
        print(f"🔎 Αναζήτηση όλων των tickers στον κλάδο: {industry_name}...")
        screener = Screener()
        
        # 1. Βάζουμε το φίλτρο Sector και Industry
        screener.set_filter(
            sector=[sector_name], 
            industry=[industry_name]
        )
        
        # 2. Παίρνουμε μόνο τη στήλη 'Ticker'
        all_tickers_df = screener.screener_view(columns=['Ticker'])
        if all_tickers_df.empty:
            print(f"⚠️ Δεν βρέθηκαν tickers για τον κλάδο: {industry_name}.")
            return []
        tickers = all_tickers_df['Ticker'].tolist()
        print(f"✅ Βρέθηκαν {len(tickers)} tickers στον κλάδο {industry_name}.")
        # Για να μην καθυστερήσει η ανάλυση, περιορίζουμε στα 50 το πολύ
        return tickers[:50]
    except Exception as e:
        print(f"❌ Σφάλμα λήψης tickers κλάδου {industry_name} ({sector_name}): {e}")
        return []
...
def load_data_from_pdf(file_path: str) -> List[Dict[str, Any]]:
    """
    v3.7: Ο "Turbo" Κινητήρας (PyMuPDF / fitz).
    Αστραπιαία φόρτωση. Δεν ψάχνει τίτλους, απλά βγάζει ΟΛΟΥΣ τους πίνακες.
    Ο "Έξυπνος Διαχωριστής" στο app.py θα τους καθαρίσει.
    """
    print(f"📄 Έναρξη σάρωσης PDF (v3.7 - PyMuPDF Engine): {file_path}")
    all_data_packages = []
    try:
        doc = fitz.open(file_path)
    except Exception as e:
        print(f"❌ Σφάλμα PyMuPDF: Δεν μπόρεσε να ανοίξει το αρχείο: {e}")
        return []
    for page_num, page in enumerate(doc):
        try:
            # Βρίσκει όλους τους πίνακες στη σελίδα
            tables = page.find_tables()
            if not tables:
                continue
            print(f"  > Σελίδα {page_num+1}: Βρέθηκαν {len(tables.tables)} πίνακες...")
            for i, table in enumerate(tables):
                try:
                    # 1. Μετατροπή σε DataFrame (μαγικό!)
                    df = table.to_pandas()
                    if df.empty:
                        continue
                    # 2. Καθαρισμός Header
                    # (Το PyMuPDF μερικές φορές επιστρέφει διπλά headers ή κενά)
                    # Βρίσκουμε την πρώτη "καθαρή" γραμμή για να την κάνουμε header
                    header_idx = 0
                    for j, row in df.iterrows():
                        if not row.isnull().all(): # Αν η γραμμή δεν είναι *τελείως* κενή
                            header_idx = j
                            break
                    new_header = df.iloc[header_idx]
                    df_data = df.iloc[header_idx+1:]
                    if df_data.empty:
                        continue # Ο πίνακας ήταν μόνο header
                    df_data.columns = new_header
                    # 3. Εξυγίανση στηλών (αφαίρεση διπλότυπων, κλπ)
                    df_sanitized = sanitize_columns(df_data)
                    # 4. Καθαρισμός τιμών (αφαίρεση $, κλπ)
                    df_clean = df_sanitized.apply(lambda col: col.map(clean_value_unstructured))
                    # 5. "Πακετάρισμα"
                    all_data_packages.append({
                        "title": f"Page_{page_num+1}_Table_{i+1}", # (Ο τίτλος είναι απλά για debug)
                        "table": df_clean
                    })
                except Exception as e:
                    print(f"  -> Warning: Αποτυχία επεξεργασίας πίνακα #{i} στη σελ. {page_num+1}: {e}")
                    continue # Προχωράμε στον επόμενο πίνακα
        except Exception as e:
            print(f"  -> CRITICAL: Αποτυχία σάρωσης σελίδας {page_num+1}: {e}")
            continue # Προχωράμε στην επόμενη σελίδα
    if not all_data_packages:
        print("⚠️ Ο Κινητήρας PyMuPDF (v3.7) δεν βρήκε πίνακες.")
        return []
    print(f"  Επιτυχία! Βρέθηκαν {len(all_data_packages)} 'πακέτα' (πίνακες).")
